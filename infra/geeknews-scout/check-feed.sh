#!/usr/bin/env bash
# check-feed.sh — GeekNews RSS scout
# Fetches GeekNews feed, filters by keywords, creates GitHub issues for matches.
#
# Env vars:
...
ISSUE_LABEL_GEEKNEWS="geeknews-scout"
...
_This issue was auto-created by geeknews-scout CronJob._
EOF
}
...
create_issue() {
    ...
    log "Creating issue: [GeekNews] ${title}"
    gh issue create \
        --repo "$REPO" \
        --title "[GeekNews] ${title}" \
        --body "$body" \
        --label "${ISSUE_LABEL_AUTOMATED},${ISSUE_LABEL_GEEKNEWS}" \
        >/dev/null

    log "Issue created: [GeekNews] ${title}"
}
...
main() {
    ...
    ensure_label "$ISSUE_LABEL_GEEKNEWS" "1d76db" "GeekNews article scout"
    ...
    # Fetch existing geeknews issues to avoid duplicates
    ...
    tracked_issue_bodies=$(gh api \
        ...
        --field "per_page=100" \
        --field "state=all" \
        --field "labels=${ISSUE_LABEL_GEEKNEWS}" \
        "repos/${REPO}/issues" \
