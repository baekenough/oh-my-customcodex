...
> **⚠️ DEPRECATED (v0.5.22, #1493)** — `daily-scout` (`.github/workflows/daily-scout.yml`, GitHub Actions + OpenAI pre-score, multi-source) supersedes this pending k8s CronJob. This component (`infra/geeknews-scout/`, keyword-regex, hada.io single-source) remains for historical reference until the daily-scout cron has a proven non-dry run. Before deletion, verify README/ARCHITECTURE/deploy script references per R020 live-state evidence.

Kubernetes CronJob that monitors [GeekNews (news.hada.io)](https://news.hada.io) for articles relevant to oh-my-customcodex and creates GitHub issues for keyword matches.
...
| `K8S_NAMESPACE` | No | `omcustomcodex` | Kubernetes namespace |
...
- Shared `github-token` secret in the `omcustomcodex` namespace
