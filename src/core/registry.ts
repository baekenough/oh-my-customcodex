/**
 * Local project registry for oh-my-customcodex
 *
 * Stores registered projects in ~/.oh-my-customcodex/projects.json so that
...
import { LEGACY_LOCKFILE_NAME, LOCKFILE_NAME } from './lockfile.js';
...
function registryDir(): string {
  ...
  const envOverride = process.env.OMCODEX_REGISTRY_DIR ?? process.env.OMCUSTOM_REGISTRY_DIR;
  ...
  return join(home, '.oh-my-customcodex');
}
 ...
 * @param version - oh-my-customcodex version installed in the project
 */
export async function registerProject(projectPath: string, version: string): Promise<void> {
  ...
  if (
    !process.env.OMCODEX_REGISTRY_DIR &&
    !process.env.OMCUSTOM_REGISTRY_DIR &&
    _registryDirOverride === undefined
  ) {
...
/**
 * Scan one or more directories for existing harness lockfiles and
 * register all discovered projects into the registry.
 *
 * This is the migration path for users who installed oh-my-customcodex before
...
export async function migrateFromLockfiles(searchDirs: string[]): Promise<number> {
  ...
  async function scan(dir: string, depth: number): Promise<void> {
    ...
    for (const lockName of [LOCKFILE_NAME, LEGACY_LOCKFILE_NAME]) {
      const entry = await parseLockFile(join(dir, lockName));
      if (entry !== null) {
