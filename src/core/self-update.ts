/**
 * Self-update check for oh-my-customcodex CLI
 * Runs before `omcodex init` in interactive sessions.
 */
...
const DEFAULT_PACKAGE_NAME = 'oh-my-customcodex';
const DEFAULT_CACHE_TTL_MS = 24 * 60 * 60 * 1000;
const DEFAULT_CACHE_PATH = join(homedir(), '.oh-my-customcodex', 'self-update-cache.json');
...
function shouldSkipSelfUpdateEnv(env: NodeJS.ProcessEnv): boolean {
  return env.OMCODEX_SKIP_SELF_UPDATE === 'true' || env.OMCUSTOM_SKIP_SELF_UPDATE === 'true';
}
...
function shouldSkipEnvironmentUpdate(argv: string[], env: NodeJS.ProcessEnv): boolean {
  ...
  if (shouldSkipSelfUpdateEnv(env)) return true;
  return false;
}
...
/**
 * Execute self-update for `omcodex update` command.
 *
...
function shouldSkipSelfUpdate(options: SelfUpdateOptions): boolean {
  ...
  if (shouldSkipSelfUpdateEnv(env)) {
    return true;
  }
