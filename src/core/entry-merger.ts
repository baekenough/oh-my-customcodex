...
const MANAGED_START = '<!-- omcodex:start -->';
const MANAGED_END = '<!-- omcodex:end -->';
const LEGACY_MANAGED_START = '<!-- omcustom:start -->';
const LEGACY_MANAGED_END = '<!-- omcustom:end -->';
...
export function parseEntryDoc(content: string): { sections: Section[] } {
  ...
  for (const line of lines) {
    ...
    if (!insideCodeBlock) {
      ...
      if (trimmed === MANAGED_START || trimmed === LEGACY_MANAGED_START) {
      ...
      if (trimmed === MANAGED_END || trimmed === LEGACY_MANAGED_END) {
