...
export interface UpdateCommandOptions {
  ...
  all?: boolean;
  /** Skip self-update of the oh-my-customcodex package */
  skipSelf?: boolean;
}
...
/**
 * Re-exec the current process with OMCODEX_SKIP_SELF_UPDATE=true so the child
...
/**
 * Handle self-update of the oh-my-customcodex package.
...
async function handleSelfUpdate(spawnFn?: SpawnSyncFn): Promise<void> {
  try {
    ...
    // not for explicit `omcodex update`.
    ...
    if (selfUpdateResult.updated) {
      ...
      if (!shouldSkipSelfUpdateEnv(process.env)) {
