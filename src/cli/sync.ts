/**
 * omcodex sync — Drift detection and snapshot export
 *
 * Usage:
 *   omcodex sync                   # Compare current state against lockfile
 *   omcodex sync --reference <dir> # Compare against external snapshot
 *   omcodex sync --export <path>   # Export current state as reusable snapshot
 */
...
async function runExport(targetDir: string, outputPath: string): Promise<void> {
  ...
  if (!result.success) {
    console.error(`\nExport failed — no ${layout.rootDir}/ directory found in current project.`);
    process.exit(1);
  }
...
export function syncCommand(program: Command): void {
  ...
  program
    ...
    .option('--export <path>', `Export current ${layout.rootDir}/ state as a reusable snapshot`)
