/**
 * CLI command handlers for the `omcodex web` subcommand group.
...
/**
 * Handler for `omcodex web start [--port 4321] [--foreground]`
...
/**
 * Handler for `omcodex web status`
...
export async function webStatusCommand(): Promise<void> {
  ...
  if (running) {
    const port = process.env.OMCODEX_PORT ?? process.env.OMCUSTOM_PORT ?? String(DEFAULT_PORT);
...
/**
 * Handler for `omcodex web open [--port 4321]`
