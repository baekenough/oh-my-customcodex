...
# Creates .codex/worktrees/feature-x with a new branch based on HEAD
...
Files under `.codex/` may be gitignored in some setups. When adding new runtime files, use `git add -f`:

```bash
git add -f .codex/agents/new-agent.md
```
