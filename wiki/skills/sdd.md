---
title: SDD (Alias)
type: skill
updated: 2026-04-12
sources:
  - .codex/skills/sdd/SKILL.md
related:
  - [[sdd-dev]]
  - [[sdd-development]]
---

# SDD (Alias)

Alias for sdd-dev — Spec-Driven Development workflow.

## Overview

This skill is an alias for `sdd-dev`. Invoking `/sdd [task description]` is equivalent to invoking `/sdd-dev [task description]`. See [[sdd-dev]] for full documentation.

## Key Details

- **Scope**: core
- **User-invocable**: yes
- **Command**: `/sdd`
- **Effort**: not specified
- **Argument hint**: `[task description or leave empty for guided workflow]`

## Relationships

- **Alias for**: [[sdd-dev]]
- **Related**: [[sdd-development]]

## Sources

- `.codex/skills/sdd/SKILL.md` — skill definition
