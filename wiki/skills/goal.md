...
`/omcustomcodex:goal <objective>` keeps a concrete user objective visible through the full task lifecycle. It captures the objective, asks only for materially blocking clarification, inspects repository context, creates a short plan for non-trivial work, executes with local conventions, verifies completion, and reports changed files plus residual risk.
