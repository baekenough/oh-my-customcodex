...
가장 저렴한 tier가 잡을 수 있는 오류를 비싼 tier로 보내지 않는다는 shift-left 원칙. hooks와 linter가 잡을 수 있는 문제를 sonnet/opus에 보내는 것은 낭비다. [[r013]] ecomode는 Compact Output으로 중간 단계와 장황한 설명을 줄여 출력 토큰을 절약하고, R023은 그와 직교적으로 검증 비용을 절약한다.
...
## Fable Over-Prescription and Detection Guard Design

R023 now distinguishes verification cost from instruction length: Fable 5 prompts should be concise because over-prescription can lower quality, while R023 still requires deterministic/cheap checks before expensive review.

When delegating Tier-1 detection guards, specify positive defects versus negative/deprecated-context explanations so grep checks do not self-block on correct deprecation notes. Workflow sanity checks also cover unescaped shell variables inside JavaScript template prompt strings, such as `${PIPESTATUS[0]}` becoming a runtime `ReferenceError`.
