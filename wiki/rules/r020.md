...
Verification, R017, and release-quality delegates must not end without a final PASS/FAIL verdict. If a verifier stops mid-step, resume it and obtain the verdict before using the report as release evidence. Claude Code v2.1.199 reduces false-success API-error reports, but R020 still requires canonical evidence such as tests, validation scripts, Git status, registry/API checks, or direct diffs.
