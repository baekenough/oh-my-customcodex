...
- **Explicit mode** (`/omcustomcodex:create-agent`): Follows a full research-driven 3-phase workflow (Research → Create → Auto-discover)
...
2. Create `.codex/agents/{name}.md` with proper R006 frontmatter
...
- **Used by**: R010 (Protected Paths rule), routing skills (dynamic fallback), `/omcustomcodex:create-agent` command
