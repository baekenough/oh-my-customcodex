...
# When /tmp/.codex-dev-stage-$PPID exists, autonomous mode cannot be activated.
...
if [ -f /tmp/.codex-dev-stage-$PPID ]; then
  stage=$(cat /tmp/.codex-dev-stage-$PPID | tr -d '[:space:]')
  if [ -z "$stage" ]; then exit 0; fi
  case "$stage" in
    plan|verify-plan|verify-impl|compound|done)
      echo "⛔ BLOCKED: Write/Edit disabled in '$stage' stage. Only allowed during 'implement' stage. Use 'echo implement > /tmp/.codex-dev-stage-$PPID' to transition."
      exit 2
      ;;
  esac
fi
