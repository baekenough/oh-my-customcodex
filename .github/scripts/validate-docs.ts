 ...
 * - Uses programmatic validation and optional OpenAI validation for docs parity
...
function extractSkillFrontmatterName(skillFile: string): string | null {
  try {
...
function toSlashAliases(skillDirName: string, frontmatterName: string | null): string[] {
  const aliases = new Set<string>([skillDirName]);

  if (!frontmatterName) {
    return [...aliases];
  }

  aliases.add(frontmatterName);

  if (frontmatterName.includes(':')) {
    const [namespace, rest] = frontmatterName.split(':', 2);
    if (namespace && rest) {
      aliases.add(`${namespace}-${rest}`);
    }
  }
...
export function validateSlashCommands(readmeContent: string, skillsDir: string): SlashCommandValidation {
  ...
  if (fs.existsSync(skillsDir)) {
    ...
    for (const skillDir of skillDirs) {
      ...
      const frontmatterName = extractSkillFrontmatterName(skillPath);
      for (const alias of toSlashAliases(skillDir.name, frontmatterName)) {
...
export function buildPrompt(
  ...
  return `당신은 oh-my-customcodex 프로젝트의 문서 검증 전문가입니다.
...
async function main() {
  ...
  try {
    ...
    if (error instanceof Error) {
      console.error(`⚠️ OpenAI API 호출 중 오류가 발생했습니다: ${error.message}`);
