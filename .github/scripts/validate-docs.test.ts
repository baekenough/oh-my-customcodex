...
describe('validateSlashCommands', () => {
  test('accepts a direct skill directory match', () => {
    const base = mkdtempSync(join(tmpdir(), 'validate-docs-slash-'));
    const skillDir = join(base, 'analysis');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), '---\nname: analysis\n---\n');

    const readme = '| `/analysis` | Analyze project |';
    const result = validateSlashCommands(readme, base);

    expect(result.valid).toEqual(['analysis']);
    expect(result.phantom).toEqual([]);

    rmSync(base, { recursive: true, force: true });
  });

  test('accepts slash aliases derived from namespaced frontmatter names', () => {
    const base = mkdtempSync(join(tmpdir(), 'validate-docs-slash-'));
  ...
  test('accepts frontmatter names even when directory name differs', () => {
    const base = mkdtempSync(join(tmpdir(), 'validate-docs-slash-'));
...
describe('buildPrompt', () => {
  ...
  test('includes readme content in the prompt', () => {
    ...
    expect(prompt).toContain('oh-my-customcodex');
