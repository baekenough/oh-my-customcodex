 ...
 * - PROJECT_CONTEXT: Optional custom context (defaults to oh-my-customcodex)
...
Commands: omcodex init, list, doctor.
  ...
  try {
    const response = await fetch(url, {
      headers: {
        ...
        'User-Agent': 'oh-my-customcodex-issue-analyzer',
      },
    });
...
async function analyzeIssueWithOpenAI(issue: IssueData): Promise<AnalysisResult> {
  console.log('🤖 Analyzing issue with OpenAI Responses API...');
  ...
  try {
    ...
    console.error('❌ Failed to analyze issue with OpenAI:', error);
...
function formatComment(analysis: AnalysisResult): string {
  ...
  comment += `---\n*이 분석은 이슈 트리아지를 돕기 위해 OpenAI API가 생성했습니다.*\n`;
...
async function main(): Promise<void> {
  ...
  // Analyze with OpenAI
  const analysis = await analyzeIssueWithOpenAI(issue);
