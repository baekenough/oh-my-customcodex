 ...
 *
 * When oh-my-customcodex releases a new version, analyzes the release changes
...
interface GitHubCreatedIssueResponse {
  number?: number;
  html_url?: string;
}
...
async function fetchTeammatesFile(path: string): Promise<string> {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'oh-my-customcodex-notify-teammates',
    },
  });
...
async function createIssue(title: string, body: string, labels: string[]): Promise<CreatedIssue> {
  ...
  const issue = (await response.json()) as GitHubCreatedIssueResponse;
  return {
...
function buildAnalysisPrompt(context: TeammatesContext): string {
  ...
  return `oh-my-customcodex의 새 릴리즈가 oh-my-teammates에 미치는 영향을 분석하세요.
기술 용어, 파일명, 코드 참조는 영어 그대로 유지하고, 설명은 한국어로 작성하세요.

## oh-my-customcodex란?
oh-my-customcodex는 oh-my-customcode의 자식 패키지로, 하네스를 GPT Codex + OMX 위에 올린 npm 패키지입니다.
핵심 구성: Agents (41개), Skills (55개), Rules (18개), Guides (22개).
변경 가능한 영역: templates, hooks, rules, skills, agents, guides.

## oh-my-teammates란?
oh-my-teammates는 oh-my-customcodex와의 호환성도 검토해야 하는 팀 협업용 확장 프로젝트입니다.
peerDependencies: { "oh-my-customcodex": ">=0.18.0" }
...
## oh-my-customcodex ${CONFIG.releaseTag} 릴리즈 변경사항
${releaseContent}
...
(예: oh-my-customcodex 자체 빌드/배포 변경, 내부 CI/CD 변경 등)
...
async function analyzeWithOpenAI(context: TeammatesContext): Promise<TeammatesAnalysis> {
  console.log('🤖 Analyzing release changes with OpenAI Responses API...');
...
function formatIssueBody(analysis: TeammatesAnalysis): string {
  ...
  let body = `## 🔄 oh-my-customcodex ${version} 릴리즈 변경사항 분석\n\n`;
  body += `> 이 이슈는 oh-my-customcodex의 새 릴리즈에서 oh-my-teammates에 적용 가능한 변경사항을 분석하여 자동 생성되었습니다.\n\n`;
  ...
  body += `### 원본 릴리즈 노트\n`;
  body += `<details>\n`;
  body += `<summary>oh-my-customcodex ${version} CHANGELOG</summary>\n\n`;
  body += `${changelogContent}\n\n`;
  body += `</details>\n\n`;

  body += `---\n*이 이슈는 oh-my-customcodex 릴리즈 시 자동 생성되었습니다.*\n`;

  return body;
}

function formatIssueTitle(analysis: TeammatesAnalysis): string {
  return `[oh-my-customcodex ${CONFIG.releaseTag}] ${analysis.summary}`;
}
