...
describe('doctor-framework', () => {
  ...
  describe('getInstalledVersion', () => {
    ...
    beforeEach(async () => {
      tempDir = await mkdtemp(join(tmpdir(), 'omcodex-fw-test-'));
    });
    ...
    it('returns null when .omcodexrc.json does not exist', async () => {
      const result = await getInstalledVersion(tempDir);
      expect(result).toBeNull();
    });

    it('returns version from .omcodexrc.json', async () => {
      await writeFile(
    ...
    it('returns null when version field is missing', async () => {
      await writeFile(join(tempDir, '.omcodexrc.json'), JSON.stringify({ configVersion: 1 }));
    ...
    it('returns null for truly empty file (zero bytes)', async () => {
      ...
      await writeFile(join(tempDir, '.omcodexrc.json'), '');
    ...
    it('returns null when file contains invalid JSON', async () => {
      await writeFile(join(tempDir, '.omcodexrc.json'), 'not-valid-json');
  ...
  describe('checkFrameworkVersion', () => {
    ...
    beforeEach(async () => {
      tempDir = await mkdtemp(join(tmpdir(), 'omcodex-fw-check-test-'));
    });
    ...
    it('returns null when no .omcodexrc.json exists', async () => {
