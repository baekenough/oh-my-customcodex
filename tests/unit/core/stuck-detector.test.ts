 ...
 *
 * The stuck-detector uses the injected OMCODEX_STUCK_HISTORY_FILE when present,
 * otherwise it falls back to /tmp/.codex-tool-history-${PPID} to track
...
function runStuckDetector(stdinInput: string, env?: Record<string, string>): Promise<ScriptResult> {
  return new Promise((resolve_) => {
    const childEnv: NodeJS.ProcessEnv = {
      ...process.env,
      OMCODEX_STUCK_HISTORY_FILE: currentHistoryFile,
      ...env,
    };
...
/** Test-local history file path to avoid cross-test interference. */
function historyFilePath(): string {
  const suffix = `${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  return `/tmp/.codex-tool-history-test-${suffix}`;
}
...
function cleanHistory(): void {
  try {
    if (currentHistoryFile) {
      require('node:fs').unlinkSync(currentHistoryFile);
    }
  } catch {
...
describe('stuck-detector.sh', () => {
  beforeEach(() => {
    currentHistoryFile = historyFilePath();
    cleanHistory();
  });
  ...
  describe('edge cases', () => {
    ...
    it('should create history file after first call', async () => {
      ...
      expect(existsSync(currentHistoryFile)).toBe(true);
    });
    ...
    it('should append one JSON entry to history file per call', async () => {
      ...
      const content = await readFile(currentHistoryFile, 'utf-8');
      const lines = content
        .trim()
        .split('\n')
        .filter((l) => l.trim() !== '');
