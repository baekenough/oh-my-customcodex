 ...
 */
beforeEach(async () => {
  tempRoot = await mkdtemp(join(tmpdir(), 'omcodex-registry-test-'));
...
describe('registerProject()', () => {
  ...
  it('creates .oh-my-customcodex directory if it does not exist', async () => {
    const { registerProject } = await import('../../../src/core/registry.js');
    const projectPath = join(tempRoot, 'dir-creation-test');

    await registerProject(projectPath, '0.79.0');

    const registryFile = join(tempRoot, '.oh-my-customcodex', 'projects.json');
...
describe('migrateFromLockfiles()', () => {
  it('imports projects with .omcodex.lock.json into the registry', async () => {
  ...
  it('imports multiple projects from nested directories', async () => {
    ...
    await writeFile(
      join(projectA, '.omcodex.lock.json'),
      JSON.stringify({ version: '0.70.0' }, null, 2),
      'utf-8'
    );
    await writeFile(
      join(projectB, '.omcodex.lock.json'),
