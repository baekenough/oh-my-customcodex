...
describe('self-update module', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omcodex-test-'));
  });
  ...
  describe('checkSelfUpdate', () => {
    ...
    it('should use default package name when not provided', () => {
      ...
      expect(capturedPackageName).toBe('oh-my-customcodex');
    });
  ...
  describe('executeSelfUpdate', () => {
    ...
    it('should skip self-update when OMCODEX_SKIP_SELF_UPDATE=true', () => {
      const options: ExecuteSelfUpdateOptions = {
        currentVersion: '1.0.0',
  ...
  describe('maybeHandleSelfUpdateForInit', () => {
    ...
    it('should return immediately when OMCODEX_SKIP_SELF_UPDATE=true', async () => {
      const options: SelfUpdateOptions = {
        currentVersion: '1.0.0',
        env: { OMCODEX_SKIP_SELF_UPDATE: 'true' },
        fetchLatestVersion: () => {
