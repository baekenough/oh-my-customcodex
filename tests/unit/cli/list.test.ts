...
describe('list command', () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'omcodex-list-test-'));
  });
  ...
  describe('output formats', () => {
    ...
    const sampleComponents: ComponentInfo[] = [
      ...
      },
      {
        ...
        type: 'language',
        path: '.codex/agents/lang-python-expert.md',
...
Description after code block.`
  ...
  describe('error handling - uncovered error paths', () => {
    ...
    it('should handle error in listCommand catch block', async () => {
      ...
      try {
        await mkdir(join(tempDir, '.codex', 'agents'), { recursive: true });
        await writeFile(join(tempDir, '.codex', 'agents', 'lang-test-agent.md'), '# Test');
  ...
  describe('getHooks', () => {
    ...
    it('should find shell script hooks', async () => {
      ...
      expect(hooks[0].path).toBe('.codex/hooks/pre-commit.sh');
    });
  ...
  describe('getContexts', () => {
    ...
    it('should find markdown context files', async () => {
      ...
      expect(contexts[0].path).toBe('.codex/contexts/ecomode.md');
