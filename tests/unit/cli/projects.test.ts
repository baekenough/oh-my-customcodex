...
beforeEach(async () => {
  ...
  const raw = await mkdtemp(join(tmpdir(), 'omcodex-projects-test-'));
  ...
  // which scans cwd/parent/options.paths for .omcodex.lock.json files.
...
describe('findProjects() — cwd inclusion (fix #546)', () => {
  ...
  it('does not affect status when cwd is not an omcodex project', async () => {
...
describe('findProjects() — parent dir inclusion (fix #546)', () => {
  it('finds a project in the parent dir when parent is outside DEFAULT_SEARCH_DIRS', async () => {
    // Structure: tempRoot/parent-project/.omcodex.lock.json
...
describe('projectsCommand() — migration mode', () => {
  it('runs migration when options.migrate is true and logs success message', async () => {
    ...
    await writeFile(
      join(projectDir, '.omcodex.lock.json'),
  ...
  it('returns success: false and includes error when migration throws', async () => {
    ...
    const badRegistryPath = join(tempRoot, '.oh-my-customcodex', 'projects.json');
