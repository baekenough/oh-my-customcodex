...
describe('writeLockFile()', () => {
  it('creates a new lock file with version and timestamps', async () => {
    ...
    const content = await readFile(join(projectDir, '.omcodex.lock.json'), 'utf-8');
  ...
  it('updates updatedAt on second write while keeping installedAt from first write', async () => {
    ...
    const first = JSON.parse(await readFile(join(projectDir, '.omcodex.lock.json'), 'utf-8'));

    await new Promise((r) => setTimeout(r, 10));

    await writeLockFile(projectDir, '0.46.0', first);
    const second = JSON.parse(await readFile(join(projectDir, '.omcodex.lock.json'), 'utf-8'));
