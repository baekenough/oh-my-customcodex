/**
 * E2E tests for `omcodex list` command
...
describe('E2E: omcodex list', () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'omcodex-e2e-list-'));
...
`;
  ...
  describe('filtering by type', () => {
    ...
    it('should list only agents when "agents" type is specified', async () => {
      ...
      // Should contain agents (official Codex-native format: {prefix}-{name})
    ...
    it('should list only skills when "skills" type is specified', async () => {
      ...
      // Should NOT contain agents (official Codex-native format: {prefix}-{name})
    ...
    it('should list all components when "all" type is specified', async () => {
      ...
      // Should contain all types (official Codex-native format: {prefix}-{name} for agents)
      expect(output).toContain('lang-golang-expert');
      expect(output).toContain('go-best-practices');
      expect(output).toContain('clean-architecture');
      expect(output).toContain('MUST-safety');
  ...
  describe('--format option', () => {
    ...
    it('should output valid JSON when --format json is specified', async () => {
      ...
      // Should contain agent data (official Codex-native format: {prefix}-{name})
    ...
    it('should output JSON with correct structure', async () => {
      ...
      // Find test-agent (official Codex-native format: {prefix}-{name})
  ...
  describe('JSON output validation', () => {
    ...
    it('should output empty array for empty list', async () => {
      // Create empty agents directory structure (official Codex-native format: .codex/agents/)
      const agentsDir = join(tempDir, '.codex', 'agents');
  ...
  describe('multiple components', () => {
    ...
    it('should list multiple agents sorted alphabetically', async () => {
      ...
      // Should contain all agents (official Codex-native format: {prefix}-{name})
